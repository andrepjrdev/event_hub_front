...
function CardTemas({tema}: CardTemaProps) {
  return (
    <div className='border-transparent border flex flex-col rounded-3xl overflow-hidden justify-between'>
      <header className='py-2 px-6 rounded-3xl bg-gray-800 shadow-lg text-white font-bold text-2xl'>Temas</header>
      <p className='p-8 text-3xl bg-transparent h-full'>{tema.descricao}</p>
      <div className="flex">
        <Link to={`/editarTema/${tema.id}`} className='w-full text-slate-100 bg-gray-800 hover:bg-gray-400 shadow-lg flex items-center justify-center py-2'>
